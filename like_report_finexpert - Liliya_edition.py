    ...
    with my_connection.cursor() as cursor:
        ...
        for PL_one in PL_all:
            ...
            if str(PL_one["Login"]) in login_list:
                worksheet_Reward.write(f'H{j}', '=\'['+agent+' рассчёт 01-'+msg_to_day+' '+month+' '+str(report_date.year)+'.xlsx]'+str(PL_one["Login"])+'\'!$V$3*1000000/20', number)
                worksheet_Reward.write(f'I{j}', '=\'['+agent+' рассчёт 01-'+msg_to_day+' '+month+' '+str(report_date.year)+'.xlsx]'+str(PL_one["Login"])+'\'!$W$3', number)
            else:
