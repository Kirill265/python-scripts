...
def report_generation(send_info):
    ...
    workbook_ = xlsxwriter.Workbook(direction+agent+' '+msg_to_day+' '+month+' '+str(report_date.year)+'.xlsx')
    ...
    worksheet_PL.set_column(0, 3, 15)
    worksheet_PL.write('E1', 'Deposit', bold_blue_)
    worksheet_PL.write('F1', 'Withdrawal', bold_blue_)
    worksheet_PL.write('G1', 'Netto', bold_blue_)
    worksheet_PL.write('H1', 'Volume Lots', bold_blue_)
    worksheet_PL.set_column(4, 7, 15)
    worksheet_PL.write('I1', 'Convertation out', bold_blue_)
    worksheet_PL.write('J1', 'Convertation in', bold_blue_)
    worksheet_PL.set_column(8, 9, 18)
    worksheet_PL.write('K1', 'P/L (+Commission)', bold_blue_)
    worksheet_PL.set_column(10, 10, 20)
    ...
    with Postgre_connection_2.cursor(cursor_factory=psycopg2.extras.DictCursor) as cursor:    
        query = """
                ...
                , ROUND(COALESCE(Dps.Deposit,0)::NUMERIC,2) - ROUND(COALESCE(-Wth.Withdrawal,0)::NUMERIC,2) AS Netto
        ...
        for OPRDS_PL in OPRDS_plus_PL:
            OPRDS_PL_dict[str(OPRDS_PL["Login"])] = {"deposit":OPRDS_PL["deposit"], "withdrawal":OPRDS_PL["withdrawal"], "netto":OPRDS_PL["netto"], "profit":OPRDS_PL["profit"]}
    ...
    with my_connection.cursor() as cursor:
        ...
        for PL_one in PL_all:
            ...
            worksheet_PL.write(f'G{j}', OPRDS_PL_dict[str(PL_one["Login"])]["netto"], number)
            worksheet_PL.write(f'H{j}', PL_one["Volume_Lots"], number)
            try:
                worksheet_PL.write(f'I{j}', convertation_dict[str(PL_one["Login"])]["out"], number)
                worksheet_PL.write(f'J{j}', convertation_dict[str(PL_one["Login"])]["in"], number)
            except KeyError:
                worksheet_PL.write(f'I{j}', 0.00, number)
                worksheet_PL.write(f'J{j}', 0.00, number)
            worksheet_PL.write(f'K{j}', OPRDS_PL_dict[str(PL_one["Login"])]["profit"], number)
        query = """
        ...
        for customerID in customerID_all:
            j += 1
            worksheet_PL.write(f'A{j}', '-',minus)
            worksheet_PL.write(f'B{j}', customerID["UTM"])
