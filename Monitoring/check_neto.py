...
def all_exposure(name = ""):
    try:
        ...
        with Postgre_connection.cursor(cursor_factory=psycopg2.extras.DictCursor) as cursor:
            ...
            """
            cursor.execute(query)
            USDNeto = cursor.fetchone()
        Postgre_connection.close()
        direction = os.path.dirname(os.path.abspath(__file__))+'\\'
        path = direction+"neto_limit.json"
        with open(path, encoding="utf-8") as fjson:
            data = json.load(fjson)
        if abs(USDNeto["inUSD"]) >= int(data["Total"])*1000000.00:
            Report = 'Нето по USD:\n$'+ '{0:,}'.format(round(USDNeto["inUSD"],2)).replace(',', ' ')
            telega_alert(u'\U000026A0'+' '+Report)
        return 'Нето по USD:\n$'+ '{0:,}'.format(round(USDNeto["inUSD"],2)).replace(',', ' ')
    except:
        return '\n\nОшибка в функции *all_exposure*'
...
def one_login(login, name = ""):
    try:
        ...
        for key in ClientValute:
            if abs(ClientValute[key]) >= int(data["ClientValute"][login][key])*1000000.00:
                Report += '\n' + key + ': $'+'{0:,}'.format(round(ClientValute[key],2)).replace(',', ' ')
        if Report != '':
            telega_alert(u'\U000026A0'+' Нето по клиенту '+login+':'+Report)
        Report = ''
        for key in ClientValute:
            Report += '\n' + key + ': $'+'{0:,}'.format(round(ClientValute[key],2)).replace(',', ' ')
        return 'Нето по клиенту '+login+':'+Report
    except:
        return '\n\nОшибка в функции *one_login*'

def by_login(name = ""):
    try:
        Report = ''
        Report += one_login('1100003787',name) + '\n\n'
        Report += one_login('1000013187',name)
        return Report
    except:
