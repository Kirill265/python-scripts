...
if not(os.path.exists(direction)):
    os.mkdir(direction)
direction = os.path.join(direction, 'A_payments')
if not(os.path.exists(direction)):
    os.mkdir(direction)
direction += '\\'
workbook = xlsxwriter.Workbook(direction+'payments 01-'+str(report_date.day)+'.'+sql_month+'.'+str(report_date.year)+'.xlsx')
...
worksheet_oper.write('A1', 'Date', bold_blue)
worksheet_oper.set_column(0, 0, 15)
worksheet_oper.write('B1', 'customer_id', bold_blue)
worksheet_oper.set_column(1, 1, 16)
worksheet_oper.write('C1', 'FIO', bold_blue)
worksheet_oper.set_column(2, 2, 25)
worksheet_oper.write('D1', 'login', bold_blue)
worksheet_oper.write('E1', 'balance', bold_blue)
worksheet_oper.write('F1', 'currency', bold_blue)
worksheet_oper.write('G1', 'volume_usd', bold_blue)
worksheet_oper.set_column(3, 6, 15)
worksheet_oper.write('H1', 'Profit without swap', bold_blue)
...
worksheet_conv.write('A1', 'customer_id', bold_blue)
worksheet_conv.set_column(0, 0, 16)
worksheet_conv.write('B1', 'FIO', bold_blue)
...
worksheet_balance.write('A1', 'Date', bold_blue)
worksheet_balance.set_column(0, 0, 15)
worksheet_balance.write('B1', 'customer_id', bold_blue)
worksheet_balance.set_column(1, 1, 16)
worksheet_balance.write('C1', 'FIO', bold_blue)
worksheet_balance.set_column(2, 2, 25)
worksheet_balance.write('D1', 'login', bold_blue)
worksheet_balance.write('E1', 'balance', bold_blue)
worksheet_balance.write('F1', 'currency', bold_blue)
...
with my_connection.cursor() as cursor:
    ...
    query = """
            ...
            WHERE u.utm_term LIKE '%payments%'
    """
    cursor.execute(query)
...
with Postgre_connection.cursor(cursor_factory=psycopg2.extras.DictCursor) as cursor:
    ...
    for convertation in convertations:
        ...
        worksheet_conv.write(f'C{k}', round(float(convertation["login_from"]),0), border_format)
        worksheet_conv.write(f'D{k}', round(float(convertation["login_to"]),0), border_format)
...
Report_Apayments = """[Расчет вознаграждения по кнопке](https://team.alfaforex.com/servicedesk/view/11606)
...
URL_TW = "https://team.alfaforex.com/servicedesk/view/11606"
message_text = ''
attached_file = direction+"payments 01-"+str(report_date.day)+"."+sql_month+"."+str(report_date.year)+".xlsx"
