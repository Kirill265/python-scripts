...
if wday in [4,5,6]:
    ...
    report_date = now - timedelta(days=(calendar.weekday(now.year, now.month, now.day)+4-4))
...
direction = 'C:/Users/Kirill_Cherkasov/Documents/Reports/Finexpert_weekly/formulas/'
...
workbook_ = xlsxwriter.Workbook(direction+'Finexpert 01-'+str(report_date.day)+' '+month+' '+str(report_date.year)+'.xlsx')
...
workbook_sum = xlsxwriter.Workbook(direction+'finexpert рассчёт.xlsx')
...
with my_connection.cursor() as cursor:
    ...
    for PL_one in PL_all:
        ...
        worksheet_PL.write(f'G{j}', PL_one["Deposit"], number)
        worksheet_PL.write(f'H{j}', PL_one["Withdrawal"], number)
        ...
        worksheet_PL.write(f'L{j}', '=VLOOKUP(A'+str(j)+',[Segregated.xlsx]Sheet1!$A$3:$K$1000000,11,0)', number)
        worksheet_PL.write(f'M{j}', '=VLOOKUP(A'+str(j)+',[Segregated1.xlsx]Sheet1!$A$3:$I$1000000,9,0)', number)
        worksheet_PL.write(f'N{j}', '=VLOOKUP(A'+str(j)+',[Segregated.xlsx]Sheet1!$A$3:$I$1000000,9,0)', number)
        worksheet_PL.write(f'O{j}', '=VLOOKUP(A'+str(j)+',[Segregated.xlsx]Sheet1!$A$3:$G$1000000,7,0)+VLOOKUP(A'+str(j)+',[Segregated.xlsx]Sheet1!$A$3:$E$1000000,5,0)', number)
        worksheet_PL.write(f'P{j}', '=M'+str(j)+'+G'+str(j)+'-H'+str(j)+'+J'+str(j)+'+K'+str(j)+'+O'+str(j)+'-N'+str(j), number)
        ...
        if str(PL_one["Login"]) in login_list:
            worksheet_Reward.write(f'F{j}', '=SUM(\'[finexpert рассчёт.xlsx]'+str(PL_one["Login"])+'\'!$Q:$Q)*1000000/20', usd_volume)
            worksheet_Reward.write(f'G{j}', '=\'[finexpert рассчёт.xlsx]'+str(PL_one["Login"])+'\'!$W$3', rub_reward)
        else:
