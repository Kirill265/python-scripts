...
def report_generation(send_info):
    ...
    worksheet_Reward.write('H1', 'Вознаграждение за конвертации, Руб', bold_blue_wrap)
    worksheet_Reward.set_column(6, 7, 25)
    ...
    worksheet_DW.write('D1', 'UTM', bold_blue_)
    worksheet_DW.set_column(3, 3, 15)
    worksheet_DW.write('E1', 'Balance before', bold_blue_)
    worksheet_DW.set_column(4, 4, 20)
    worksheet_DW.write('F1', 'Deposit / Withdrawal', bold_blue_)
    worksheet_DW.write('G1', 'Time', bold_blue_)
    worksheet_DW.set_column(5, 6, 25)
    ...
    with my_connection.cursor() as cursor:
        ...
        for PL_one in PL_all:
            j += 1
            Login_utm_dict[str(PL_one["Login"])] = {"utm":PL_one["UTM"]}
        ...
        for DandW in DepositAndWithdrawal:
            ...
            worksheet_DW.write(f'D{s}', Login_utm_dict[str(DandW["Login"])]["utm"])
            worksheet_DW.write(f'E{s}', DandW["Balance_before"],number)
            worksheet_DW.write(f'F{s}', DandW["Profit"],number)
