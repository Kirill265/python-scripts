...
def check_CSV_func(direction,CSV_file,csv_list,repType,OGRN,report_date,shag):
    global count_deal
    global new_deal
    global cancel_deal
    global code_list
    global count_error
    global count_error_context
    global uid_list
    global check
    global progress
    ...
    myXBRL_context = open(direction+'\\temp1_XBRL_'+OGRN+'_ep_nso_purcb_'+repType+'_10d_reestr_0420417_'+report_date.replace("-","")+'.xml','w',encoding='utf-8')
    ...
    myXBRL_context.write("<link:schemaRef xlink:type=\"simple\" xlink:href=\"http://www.cbr.ru/xbrl/nso/purcb/rep/2019-12-31/ep/ep_nso_purcb_"+repType+"_10d_reestr_0420417.xsd\" />")
    myXBRL_purcb = open(direction+'\\temp2_XBRL_'+OGRN+'_ep_nso_purcb_'+repType+'_10d_reestr_0420417_'+report_date.replace("-","")+'.xml','w',encoding='utf-8')
    ...
    for deal in csv_list:
        ...
        identifier = OGRN
        period = report_date
        ID_strokiTypedname = deal.get("Идентификатор строки",str(count_str))
        ID_SdelkiTypedName = deal.get("Идентификатор сделки","")
        ...
        Tip_identif_klienta_VnebirzhSdelkaEnumerator = 'mem-int:'+taxonomy_func('Тип идентификатора клиента','IDfizicheskogoLiczaMember',deal,taxonomy_dict)
        ...
        Tip_identif_kontr_VnebirzhSdelkaEnumerator = 'mem-int:'+taxonomy_func('Тип идентификатора контрагента','IDfizicheskogoLiczaMember',deal,taxonomy_dict)
        ...
        count_deal += 1
        if not re.fullmatch(r'\d+', ID_strokiTypedname):
            error_list += uid + ":\t" + ID_strokiTypedname + "\tНекорректный показатель ID_strokiTypedname\n"
        if not re.fullmatch(r'\d+', ID_SdelkiTypedName):
            error_list += uid + ":\t" + ID_SdelkiTypedName + ":\tНекорректный показатель ID_SdelkiTypedName\n"
        ...
        if Tip_identif_klienta_VnebirzhSdelkaEnumerator != 'mem-int:IDfizicheskogoLiczaMember':
        ...
        if not re.fullmatch(r'\d{2}\s\d{2}\s\d{6}/1', Identifikator_klienta_po_vneb_sdelke_Rek_kl_vneb_sd):
        ...
        if Tip_identif_kontr_VnebirzhSdelkaEnumerator != 'mem-int:IDfizicheskogoLiczaMember':
            error_list += uid + ":\t" + Tip_identif_kontr_VnebirzhSdelkaEnumerator + ":\tНекорректный показатель Tip\n"
        if not re.fullmatch(r'\d{2}\s\d{2}\s\d{6}/1', Identr_kontr_po_vneb_sdelke_Rek_kontr_po_vneb_sd):
    ...
    if not re.fullmatch(r'Решение единственного участника ООО "Альфа-Форекс" от \d{2} сентября \d{4} года', Osnispobyaz):
...
Thread_1 = Thread(target=progress_bar, args=())
Thread_2 = Thread(target=check_CSV_func, args=(direction,CSV_file,csv_list,repType,OGRN,report_date,shag))
...
if result_dir != '/':
    ...
    with open(result_dir+'XBRL_'+OGRN+'_ep_nso_purcb_'+repType+'_10d_reestr_0420417_'+report_date.replace("-","")+'.xml','w',encoding='utf-8') as myXBRL:
        with open(direction+'\\temp1_XBRL_'+OGRN+'_ep_nso_purcb_'+repType+'_10d_reestr_0420417_'+report_date.replace("-","")+'.xml','r',encoding='utf-8') as temp1_XBRL:
            for line in temp1_XBRL:
                myXBRL.write(line)
        with open(direction+'\\temp2_XBRL_'+OGRN+'_ep_nso_purcb_'+repType+'_10d_reestr_0420417_'+report_date.replace("-","")+'.xml','r',encoding='utf-8') as temp2_XBRL:
