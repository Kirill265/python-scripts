    ...
    with open(getXBRL,'r',encoding='utf-8') as f_orig:
        ...
        for line in f_orig:
            ...
            if '<purcb-dic:' in line:
                ...
                try:
                    if VnebSdelka[0:2] != DataSdelki[8:10] or VnebSdelka[3:5] != DataSdelki[5:7] or VnebSdelka[6:10] != DataSdelki[0:4] or VnebSdelka[23] != NaprSdelki[8]:
                        error_list += context_ref + ":\tНекорректный уникальный код\n"
                except:
                    error_list += context_ref + ":\tНекорректный уникальный код\n"
            if error_list != "":
                if 'explicitMember' not in error_list:
                    count_error_deal += 1
                count_error_context += 1
                ferror_temp.write(error_list)
                count_error += error_list.count("\n")
